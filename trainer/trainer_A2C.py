...
class TrainerA2C:
    def __init__(self, env, agent, episodes=500, max_steps=500, save_path="result/a2c",
        ...
        for episode in range(1, self.episodes + 1):
            ...
            for step in range(self.max_steps):
                ...
                if done or step == self.max_steps - 1:
                    print(f"[A2C] Episode {episode}/{self.episodes} - Reward: {total_reward} - Steps: {step + 1}")
                    self.step_used.append(step + 1)
                    break

            self.reward_history.append(total_reward)
            # 每回合结束后记录 loss 和策略分布
            if hasattr(self.agent, "get_last_metrics"):
                loss, policy = self.agent.get_last_metrics()
